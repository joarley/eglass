...
/*
    ...
    //Custom Dialog
    kendo.ui.ExtCustomDialog = {
        show: function(options) {
            return new $.Deferred(function(deferred) {
                if ($("#extCustomDialog").length > 0) {
                    $("#extCustomDialog").parent().remove();
                }
                options = $.extend({
                    width: "300px",
                    height: "100px",
                ...
                $.each(buttons, function(idx, value) {
                    options.buttons.push({
                        name: value,
                        click: function(e) {
                            $("#extCustomDialog").data("kendoExtDialog").close();
                            deferred.resolve({ button: value });
                        }
                    });
                });

                $(document.body).append(kendo.format("<div id='extCustomDialog' style='position:relative;'><div style='position:absolute;left:10px;top:10px;' class='{0}'></div><div style='display:inline-block;margin-left:45px;'>{1}</div></div>", options.icon, options.message));
                $("#extCustomDialog").kendoExtDialog(options);
                $("#extCustomDialog").parent().find("div.k-window-titlebar div.k-window-actions").empty();
                $("#extCustomDialog").data("kendoExtDialog").center().open();
            });
        },
